...
public class ExclusiveTimeFunctions {
    ...
    public int[] exclusiveTime(int n, List<String> logs) {
        ...
        for (String log : logs) {
            //当前序号
            int curNo = Integer.parseInt(log.substring(0, log.indexOf(':')));
            //当前标志位
            String curFlag = log.substring(log.indexOf(':') + 1, log.lastIndexOf(':'));
            //当前时间戳
            int curTime = Integer.parseInt(log.substring(log.lastIndexOf(':') + 1));
            //程序的开始
            if ("start".equals(curFlag)) {
                //有程序运行
                if (!stack.isEmpty()) {
                    //记录前一个程序的运行时间
                ...
                //加入新程序
                stack.push(new int[]{curNo, curTime});
            } else {
                //栈顶程序结束
                int[] t = stack.pop();
                //计算运行时间
                res[t[0]] += curTime - t[1] + 1;
                //还有程序运行
                if (!stack.isEmpty()) {
