 ...
 * (4)这里给出几个有效单词的例子："a-b."、"afad"、"ba-c"、"a!" 和 "!" 。
 *
...
public class NumValidWords {
    ...
    public int countValidWords(String sentence) {
        //按照空格切分
        String[] data = sentence.split("\\s+");
        //结果
        int res = 0;
        //遍历
        for (String s : data) {
            //判断是否有效
            if (!s.isEmpty() &&valid(s)){
                res++;
            }
        }
        return res;
    ...
    private boolean valid(String s) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            //当前是‘-’符号
            if (c =='-'){
                //已经存在一个或者在开头或者结尾处或者后面不是字母
                if (flag || i == 0 || i == n-1 ||!Character.isAlphabetic(s.charAt(i+1)))
                    return false;
                //出现过满足条件的‘-’
                flag = true;
            }
