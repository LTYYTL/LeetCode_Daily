...
public class ExpressiveWords {
    ...
    public int expressiveWords(String s, String[] words) {
        ...
        for (String word : words) {
            // 计算是否合格
            res += stretchyWord(s.toCharArray(), word.toCharArray()) ? 1 : 0;
        }

        return res;
    }

    private boolean stretchyWord(char[] s, char[] word) {
        // word长度超过s长度
        ...
        // 遍历，在每次循环时改变基础字符索引
        while ((base = si) < s.length && wi < word.length) {
            // 记录字符连续出现次数
            int c1 = 0;
            int c2 = 0;
            // 在s中字符个数
            while (si < s.length && s[si] == s[base]) {
                c1++;
                si++;
            }

            // 在word中字符个数
            while (wi < word.length && word[wi] == s[base]) {
                c2++;
                wi++;
            }
        ...
        // 只有s和word都遍历完毕，才返回true
        return si == s.length && wi == word.length;
